...
export const people = customerSchema.table(
  "people",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (table) => [
    ...
    check(
      "people_role_check",
      sql`${table.role} IS NULL OR ${table.role} IN ('owner', 'admin')`,
    ),
    // Partial unique index: only 1 owner per tenant
    uniqueIndex("people_tenant_owner_unique")
      .on(table.tenantId)
...
export const tenantPeopleFields = customerSchema.table(
  "tenant_people_fields",
  {
  ...
  (table) => [
    unique("tenant_people_fields_tenant_name_unique").on(
      table.tenantId,
      table.name,
    ),
...
export const tenantPeopleFieldValues = customerSchema.table(
  "tenant_people_field_values",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    ...
    fieldId: bigint("field_id", { mode: "number" })
      .notNull()
      .references(() => tenantPeopleFields.id, { onDelete: "cascade" }),
    value: text("value"),
    createdAt: timestamp("created_at").notNull().defaultNow(),
    updatedAt: timestamp("updated_at").notNull().defaultNow(),
  },
  (table) => [
    unique("tenant_people_field_values_person_field_unique").on(
      table.personId,
      table.fieldId,
    ),
...
export const tenantUsers = customerSchema.table(
  "tenant_users",
  {
    personId: uuid("person_id")
      .primaryKey()
  ...
  (table) => [index("tenant_users_person_id_idx").on(table.personId)],
);
...
export const tenantMembers = customerSchema.table(
  "tenant_members",
  {
    personId: uuid("person_id")
      .primaryKey()
  ...
  (table) => [index("tenant_members_person_id_idx").on(table.personId)],
);
...
export const tenantTeams = customerSchema.table(
  ...
  (table) => [index("tenant_teams_tenant_id_idx").on(table.tenantId)],
);
...
export const tenantTeamMembers = customerSchema.table(
  "tenant_team_members",
  {
    ...
    teamId: bigint("team_id", { mode: "number" })
      .notNull()
      .references(() => tenantTeams.id, { onDelete: "cascade" }),
    userId: uuid("user_id")
      .notNull()
      .references(() => tenantUsers.personId, { onDelete: "cascade" }),
    tenantId: uuid("tenant_id")
      .notNull()
      .references(() => tenants.id, { onDelete: "cascade" }),
    role: text("role"),
...
export const tenantTeamMemberSkills = customerSchema.table(
  ...
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    teamMemberId: bigint("team_member_id", { mode: "number" })
      .notNull()
      .references(() => tenantTeamMembers.id, { onDelete: "cascade" }),
    skillId: bigint("skill_id", { mode: "number" })
      .notNull()
      .references(() => tenantTeamSkills.id, { onDelete: "cascade" }),
    tenantId: uuid("tenant_id")
      .notNull()
      .references(() => tenants.id, { onDelete: "cascade" }),
    createdAt: timestamp("created_at").notNull().defaultNow(),
  ...
  (table) => [
    unique("tenant_team_member_skill_unique").on(
      table.teamMemberId,
      table.skillId,
    ),
    index("tenant_team_member_skills_team_member_id_idx").on(
      table.teamMemberId,
    ),
...
export const tenantSkillIncompatibility = customerSchema.table(
  ...
  {
    skillId1: bigint("skill_id_1", { mode: "number" })
      .notNull()
      .references(() => tenantTeamSkills.id, { onDelete: "cascade" }),
    skillId2: bigint("skill_id_2", { mode: "number" })
      .notNull()
      .references(() => tenantTeamSkills.id, { onDelete: "cascade" }),
    tenantId: uuid("tenant_id")
      .notNull()
  ...
  (table) => [
    unique("tenant_skill_incompatibility_pk").on(
      table.skillId1,
      table.skillId2,
      table.tenantId,
    ),
...
export const tenantEventTemplates = customerSchema.table(
  ...
  (table) => [index("tenant_event_templates_tenant_id_idx").on(table.tenantId)],
);
...
export const tenantEventTemplateSlots = customerSchema.table(
  "tenant_event_template_slots",
  {
    ...
    templateId: bigint("template_id", { mode: "number" })
      .notNull()
      .references(() => tenantEventTemplates.id, { onDelete: "cascade" }),
    tenantId: uuid("tenant_id")
      .notNull()
      .references(() => tenants.id, { onDelete: "cascade" }),
    teamId: bigint("team_id", { mode: "number" })
...
export const tenantEvents = customerSchema.table(
  "tenant_events",
  {
    ...
    templateId: bigint("template_id", { mode: "number" }).references(
      () => tenantEventTemplates.id,
      { onDelete: "set null" },
    ), // nullable, reference to original template
    name: text("name").notNull(),
...
export const tenantEventSlots = customerSchema.table(
  "tenant_event_slots",
  {
    ...
    eventId: bigint("event_id", { mode: "number" })
      .notNull()
      .references(() => tenantEvents.id, { onDelete: "cascade" }),
    tenantId: uuid("tenant_id")
      .notNull()
      .references(() => tenants.id, { onDelete: "cascade" }),
    teamId: bigint("team_id", { mode: "number" })
...
export const tenantEventAssignments = customerSchema.table(
  "tenant_event_assignments",
  {
    ...
    slotId: bigint("slot_id", { mode: "number" })
      .notNull()
      .references(() => tenantEventSlots.id, { onDelete: "cascade" }),
    tenantId: uuid("tenant_id")
      .notNull()
      .references(() => tenants.id, { onDelete: "cascade" }),
    userId: uuid("user_id")
...
export const songs = customerSchema.table(
  "songs",
  {
    ...
    originalKeyId: integer("original_key_id")
      .notNull()
      .references(() => keys.id),
  ...
  (table) => [
    ...
    check(
      "songs_release_year_check",
      sql`${table.releaseYear} >= 1000 AND ${table.releaseYear} <= 3000`,
    ),
    check("songs_bpm_check", sql`${table.bpm} >= 40 AND ${table.bpm} <= 300`),
    check(
      "songs_duration_check",
      sql`${table.duration} >= 60 AND ${table.duration} <= 900`,
    ),
    check(
      "songs_chords_and_lyrics_not_empty",
      sql`length(${table.chordsAndLyrics}) > 0 AND length(${table.chordsAndLyrics}) <= 4000`,
    ),
    check(
      "songs_time_sig_num_check",
      sql`${table.timeSigNum} >= 1 AND ${table.timeSigNum} <= 32`,
    ),
    check(
      "songs_time_sig_den_check",
      sql`${table.timeSigDen} IN (1, 2, 4, 8, 16, 32)`,
    ),
    check(
      "songs_recommended_male_offset_check",
      sql`${table.recommendedMaleOffset} >= -11 AND ${table.recommendedMaleOffset} <= 11`,
    ),
    check(
      "songs_recommended_female_offset_check",
      sql`${table.recommendedFemaleOffset} >= -11 AND ${table.recommendedFemaleOffset} <= 11`,
    ),
  ],
);
...
export const tenantPeopleFieldsRelations = relations(
  tenantPeopleFields,
  ({ one, many }) => ({
    tenant: one(tenants, {
...
export const tenantPeopleFieldValuesRelations = relations(
...
export const tenantTeamMembersRelations = relations(
  tenantTeamMembers,
  ({ one, many }) => ({
...
export const tenantTeamSkillsRelations = relations(
  tenantTeamSkills,
  ({ one, many }) => ({
    tenantTeam: one(tenantTeams, {
    ...
    tenantTeamMemberSkills: many(tenantTeamMemberSkills),
    incompatibilityAsSkill1: many(tenantSkillIncompatibility, {
      relationName: "skill1",
    }),
    incompatibilityAsSkill2: many(tenantSkillIncompatibility, {
      relationName: "skill2",
    }),
  }),
);

export const tenantTeamMemberSkillsRelations = relations(
...
export const tenantSkillIncompatibilityRelations = relations(
...
export const tenantEventTemplatesRelations = relations(
...
export const tenantEventTemplateSlotsRelations = relations(
...
export const tenantEventsRelations = relations(
  tenantEvents,
  ({ one, many }) => ({
    tenant: one(tenants, {
...
export const tenantEventSlotsRelations = relations(
  tenantEventSlots,
  ({ one, many }) => ({
...
export const tenantEventAssignmentsRelations = relations(
...
export type TenantPeopleFieldValue =
  typeof tenantPeopleFieldValues.$inferSelect;
export type NewTenantPeopleFieldValue =
  typeof tenantPeopleFieldValues.$inferInsert;
...
export type NewTenantTeamMemberSkill =
  typeof tenantTeamMemberSkills.$inferInsert;

export type TenantSkillIncompatibility =
  typeof tenantSkillIncompatibility.$inferSelect;
export type NewTenantSkillIncompatibility =
  typeof tenantSkillIncompatibility.$inferInsert;
...
export type TenantEventTemplateSlot =
  typeof tenantEventTemplateSlots.$inferSelect;
export type NewTenantEventTemplateSlot =
  typeof tenantEventTemplateSlots.$inferInsert;
...
export type NewTenantEventAssignment =
  typeof tenantEventAssignments.$inferInsert;
