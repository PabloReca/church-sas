...
<div
	...
	class="flex relative rounded-lg telephone-component border border-border transition-colors w-full {isFocused ? 'ring-1 ring-primary border-primary' : ''}"
>
	<div class="flex" use:clickOutsideAction={() => clickOutside && closeDropdown()}>
		<button
			class="relative shrink-0 overflow-hidden z-10 whitespace-nowrap inline-flex items-center py-2.5 px-4 font-medium text-center text-muted-foreground bg-input border-0 rounded-l-lg hover:bg-muted focus:outline-none transition-colors border-r border-border cursor-pointer"
			type="button"
			role="combobox"
		...
		>
			{#if selectedCountry}
				<div class="inline-flex items-center text-left">
					...
					<span class="text-foreground">+{selectedCountryDialCode}</span>
				</div>
			{:else}
				Selecciona un país
			{/if}
		...
		{#if isOpen}
			<div id="dropdown-countries" class="absolute z-50 w-full bg-card border border-border rounded-md shadow-lg overflow-hidden translate-y-11">
				<div class="sticky top-0 bg-card border-b border-border">
					<input
						bind:this={searchInput}
						type="text"
						class="px-4 py-2 focus:outline-none w-full bg-card text-foreground placeholder-muted-foreground"
				...
				<div class="max-h-48 overflow-y-auto countries-scroll">
					{#each sortCountries(normalizedCountries, searchText) as country (country.id)}
						...
						<button
							type="button"
							class="inline-flex py-2 px-4 w-full hover:bg-muted transition-colors cursor-pointer {isActive ? 'bg-muted' : 'text-foreground'}"
						...
						>
							...
							<span class="text-muted-foreground shrink-0">+{country.dialCode}</span>
						</button>
					{/each}
				</div>
			</div>
	...
	<TelInput
		...
		bind:valid
		{options}
		class="rounded-r-lg block w-full p-2.5 focus:outline-none border-0 bg-input text-foreground placeholder-muted-foreground transition-colors"
